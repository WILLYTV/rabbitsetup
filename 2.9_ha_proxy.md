...
### 1. Remova os nodes 03c e 04c do cluster RabbitMQ
Antes de parar o serviço, execute em um node ativo do cluster (ex: 00c):
```bash
...
### 2. Desabilite o RabbitMQ nos nodes 03c e 04c
Após remover do cluster, pare e desabilite o serviço:
```bash
sudo systemctl stop rabbitmq-server
sudo systemctl disable rabbitmq-server
```

### 2. Instale e configure o HAProxy normalmente nesses nodes
- Use as portas padrão (5672, 15672, 15671) para o HAProxy.
- Não é necessário alterar as portas do RabbitMQ nos outros nodes (00c, 01c, 02c).

### 3. Configure o backend do HAProxy para encaminhar para todos os nodes do cluster RabbitMQ ativos (00c, 01c, 02c).
